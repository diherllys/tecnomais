 ...
 * @author Usuario
 */
public class BuscaProdutoEntradaView extends javax.swing.JDialog {
     ...
     */
    public BuscaProdutoEntradaView(java.awt.Frame parent, boolean modal, String consulta) {
        super(parent, modal);
        initComponents();
        ajustarTabela();
        if (tfBuscar.getText().contains("*")) {
            preecherTabelaLike(consulta.replace("*", ""));
        } else {
    ...
    private void initComponents() {
        ...
        tbTabela.setModel(new javax.swing.table.DefaultTableModel(
            ...
            new String [] {
                "Codigo", "Descrição", "Classe", "Qtde. Atual", "Preço de Custo", "Unidade", "Id"
            }
        ) {
    ...
    private void tbTabelaMousePressed(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_tbTabelaMousePressed
        ...
        prod.setPrecoDeCusto(Double.parseDouble(tbTabela.getValueAt(linha, 4).toString().replace(",", ".")));
...
//        atalhos(evt);
    ...
    private void tfBuscarActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_tfBuscarActionPerformed
        if (tfBuscar.getText().contains("*")) {
            preecherTabelaLike(tfBuscar.getText().replace("*", ""));
        } else {
    ...
    private void tbTabelaKeyPressed(java.awt.event.KeyEvent evt) {//GEN-FIRST:event_tbTabelaKeyPressed
        int linha = tbTabela.getSelectedRow();
        int id = Integer.parseInt(tbTabela.getValueAt(linha, 0).toString());
        ProdutoEntity ce = new ProdutoEntity();
        produto = new ProdutoEntity();
        ce.setId(id);
        bSelecionar.setEnabled(true);
        for (ProdutoEntity c : pd.selecionaProduto(ce.getId())) {
            produto = c;
        }
    ...
    public static void main(String args[]) {
        ...
        try {
            ...
            java.util.logging.Logger.getLogger(BuscaProdutoEntradaView.class.getName()).log(java.util.logging.Level.SEVERE, null, ex);
        ...
        java.awt.EventQueue.invokeLater(new Runnable() {
            public void run() {
                BuscaProdutoEntradaView dialog = new BuscaProdutoEntradaView(new javax.swing.JFrame(), true, null);
    ...
    public void preecherTabela(String consulta) {
        try {
            ...
            for (ProdutoEntity p : pd.listar(consulta + "%")) {
                tb.addRow(new Object[]{p.getCodigoProduto(), p.getNome(), p.getClasse(), p.getQntAtual(), new DecimalFormat("0.00").format(p.getPrecoDeCusto()), p.getUnidadeDeVenda(), p.getId()});
            }
        } catch (Exception e) {
        }
    }

    public void preecherTabelaLike(String consulta) {
        try {
            DefaultTableModel tb = (DefaultTableModel) tbTabela.getModel();
            tb.setNumRows(0);
            for (ProdutoEntity p : pd.listar("%" + consulta + "%")) {
